...
# Usage: fleet-upx.sh VER=1.18.21 NODES="miao@host1 miao@host2"
...
if [ "$STATUS_MODE" -eq 1 ]; then
    ...
    for node in $NODES; do
        if sshpass -p 0 ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=no "$node" "echo OK" 2>/dev/null; then
            echo "  $node: reachable"
        else
...
if [ -z "$VER" ] || [ -z "$NODES" ]; then
    echo "Usage: fleet-upx.sh VER=x.y.z NODES=\\"host1 host2\\""
    echo "       fleet-upx.sh --status NODES=\\"host1 host2\\""
    exit 1
fi
...
for node in $NODES; do
    ...
    # Transfer
    if ! sshpass -p 0 scp -o ConnectTimeout=10 -o StrictHostKeyChecking=no \
    ...
    if RESULT=$(sshpass -p 0 ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no \
