...
if check_bun_marker; then
    ...
    log "detected native Go binary (no Bun marker), using glibc-runner path"
    # Native Go binaries: use direct copy with simple wrapper
    # The binary is already glibc-linked, can run via glibc-runner
    mkdir -p "$RUNTIME_DIR"
    # For native Go binaries, copy directly - they work with glibc-runner
